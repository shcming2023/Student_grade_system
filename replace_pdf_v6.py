...
new_function = r'''def generate_report_card_pdf(student_id, exam_session_id, template_id=None):
    """生成学生成绩单PDF - Refactored v6 (Optimized)"""
    print(f"DEBUG: Generating PDF for student {student_id}, session {exam_session_id}, template {template_id} - NEW LOGIC v6")
    try:
        # 注册中文字体
        try:
        ...
        header_center_style.fontSize = 16 # Larger
        header_center_style.alignment = 1 # Center
        # header_center_style.fontName = 'Helvetica-Bold' # Can't mix font unless reg
        ...
        if system_logo_path:
             ...
             if os.path.exists(abs_logo_path):
                 try:
                     # Create Image object, Larger height 25mm
        ...
        for exam_reg, template, subject in registrations:
            ...
            # Increased logo size means right column might need more space if wide logo
            header_data = [[
            ...
            for q in questions:
                ...
                # Knowledge Point: No truncation, auto-wrap
            ...
            # Box style - Half Page Height (approx 130mm)
            ...
            # Footer (Repeated on new page if needed, but handled by doc template usually? 
            # SimpleDocTemplate flows. We can add footer manually here.)
            story.append(Spacer(1, 10))
...
if start_line != -1:
    ...
    with open(file_path, 'w') as f:
        f.writelines(lines)
    print("Function replaced with v6.")
else:
