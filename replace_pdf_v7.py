...
new_function = r'''def generate_report_card_pdf(student_id, exam_session_id, template_id=None):
    """生成学生成绩单PDF - Refactored v7 (Small Fonts)"""
    print(f"DEBUG: Generating PDF for student {student_id}, session {exam_session_id}, template {template_id} - NEW LOGIC v7")
    try:
        # 注册中文字体
        try:
        ...
        for exam_reg, template, subject in registrations:
            ...
            for q in questions:
                ...
                # Use table_style (small)
                detail_rows.append([
                    Paragraph(f"Q{q.question_number}", table_style),
                    Paragraph(q.module or '-', table_style),
                    Paragraph(kp, table_style),
                    Paragraph(f'<font color="{color}">{is_correct_text}</font>', table_style)
                ])
            ...
            detail_table.setStyle(TableStyle([
                ...
                ('FONTSIZE', (0, 0), (-1, -1), 8), # Header Font Size (slightly bigger than content)
...
if start_line != -1:
    ...
    with open(file_path, 'w') as f:
        f.writelines(lines)
    print("Function replaced with v7.")
else:
